...
def upload_media():
    ...
    try:
        ...
        # Handle token_drive - could be empty JSON or actual token
        token_drive_data = account.get('token_drive', '{}')
        
        # If token is empty JSON, return success without uploading to Drive
        if not token_drive_data or token_drive_data == "{}":
            conn.close()
            return jsonify({"message": "File queued for upload (no Drive token)"}), 200

        try:
            token_drive = json.loads(token_drive_data)

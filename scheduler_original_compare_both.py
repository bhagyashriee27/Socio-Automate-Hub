...
def run_scheduler_once():
    ...
    Compares earliest next_post_time from instagram and telegram tables and runs the appropriate script.
    """
    conn = get_db_connection()
    if not conn:
    ...
    # Compare earliest next_post_time from instagram and telegram
    instagram_time = get_earliest_post_time('instagram')
    telegram_time = get_earliest_post_time('telegram')

    print("Comparing earliest post times:")
    ...
    if not instagram_time and not telegram_time:
        print("No scheduled posts found in either table. Exiting.")
        return
    elif not instagram_time:
        print("No Instagram posts scheduled. Running post_on_telegram.py...")
        sleep(3)
        subprocess.run(["python", os.path.join(os.path.dirname(__file__), "post_on_telegram.py")])
    elif not telegram_time:
        print("No Telegram posts scheduled. Running post_reel_loop.py...")
        sleep(3)
        subprocess.run(["python", os.path.join(os.path.dirname(__file__), "post_reel_loop.py")])
    else:
        # Convert times to UTC for comparison to avoid timezone issues
        instagram_time_utc = instagram_time.astimezone(pytz.utc) if instagram_time else None
        telegram_time_utc = telegram_time.astimezone(pytz.utc) if telegram_time else None
        if instagram_time_utc <= telegram_time_utc:
            print("Instagram has the earliest post time. Running post_reel_loop.py...")
            sleep(3)
            subprocess.run(["python", os.path.join(os.path.dirname(__file__), "post_reel_loop.py")])
        else:
            print("Telegram has the earliest post time. Running post_on_telegram.py...")
            sleep(3)
            subprocess.run(["python", os.path.join(os.path.dirname(__file__), "post_on_telegram.py")])
